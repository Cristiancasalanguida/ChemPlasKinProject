...
public:
    ...
    (
            ...
            std::string plasmaHeatModel = " " // "Flitti" or "Detailed" (default: "Detailed")
    )
    : m_BoltzmannSpecies(BoltzmannSpecies)
    {
        ...
        O2Index = findSpeciesIndex("O2");
        H2Index = findSpeciesIndex("H2");
        OIndex = findSpeciesIndex("O");
    ...
    void eval(double t, double* y, double* ydot, double* p) override {
        ...
        if (nonThermal) {
            ...
            *Ep_dot = P_ext;
            ...
            // Heat loss through radial thermal conduction from the hot discharge kernel
            // to the ambient environment
            double q_loss = 0.0;
            q_loss = (heatLoss ? C0 * m_transport->thermalConductivity() * (temperature - T0) : 0.0);
            ...
            if (detailPlasmaHeatModel) {
                ...
                double evibO2_sum = 0.0;
                double evibNO_sum = 0.0;
                double evibNH3_sum = 0.0;
                double FGH_sum = 0.0;
                ...
                for (const auto &[i, e_vib]: evib_O2_map) {
                    evibO2_sum += e_vib * m_q_net[i] * Avogadro * ElectronCharge;
                }
                for (const auto &[i, e_vib]: evib_NO_map) {
                    evibNO_sum += e_vib * m_q_net[i] * Avogadro * ElectronCharge;
                }
                for (const auto &[i, e_vib]: evib_NH3_map) {
                    evibNH3_sum += e_vib * m_q_net[i] * Avogadro * ElectronCharge;
                }

                // Vibrational relaxation time scale of N2(v)
                double T_1_3 = pow(temperature, -0.33333333); // T^(-1/3)
                double tauVT_N2 = OneAtm / m_gas->pressure() / (
                        // Millikan, R. C., & White, D. R. (1963). Systematics of vibrational relaxation.
                        // The Journal of chemical physics, 39(12), 3209-3213.
                        m_gas->moleFraction(N2Index) / exp(220 * (T_1_3 - 0.03) - 18.42) +
                        // Colgan, S. J., & Levitt, B. P. (1967). Vibrational relaxation of nitrogen by various collision partners at 2500° K.
                        // Transactions of the Faraday Society, 63, 2898-2905.
                        // A=162.3 is inferred from P*tau(N2-O2) = 1.2E-5 atm*sec at 2500 K
                        m_gas->moleFraction(O2Index) / exp(162 * (T_1_3 - 0.03) - 18.42) +
                        // D. R. White, J. Chem. Physics, 1967, 46, 2016. Vibrational Relaxation of N2 in N2-H2 Mixtures
                        m_gas->moleFraction(H2Index) / exp(80.6 * (T_1_3 - 0.0176) - 18.42) +
                        // N.A. Popov, J. Phys. D: Appl. Phys. 44 (2011). Eq(8), k_VT = 4.5E-15*(T/300)^2.1 cm^3/s
                        OneAtm * 0.00205 * m_gas->moleFraction(OIndex) * pow(temperature, 1.1)
                );

                // Vibrational relaxation time scale of O2(v)
                double tauVT_O2 = OneAtm / m_gas->pressure() / (
                        // Millikan, R. C., & White, D. R. (1963). Systematics of vibrational relaxation.
                        // White, D. R., & Millikan, R. C. (1964). Vibrational relaxation in air.
                        m_gas->moleFraction(O2Index) / exp(129 * (T_1_3 - 0.03) - 18.42) +
                        m_gas->moleFraction(N2Index) / exp(124.3 * (T_1_3 - 0.03) - 18.42) +
                        m_gas->moleFraction(H2Index) / exp(42 * (T_1_3 - 0.03) - 18.42)
                        //                m_gas->moleFraction(OIndex)
                );

                // Here only N2 V-T relaxation is considered
                double RVT_N2 = eVibN2 / tauVT_N2;
                ...
                *Ep_dot = ext_sum;
                ...
                if (constPressure) {
                    ...
                    *dTdt = (-hdot_vol + eth_sum + RVT_N2 + evibO2_sum - q_loss) / (rho * cp);
                } else {
                    // *dTdt = - udot_vol / (rho * cv);
                    *HE_dot = -udot_vol + eth_sum + RVT_N2 + evibO2_sum;
                    *dTdt = (-udot_vol + eth_sum + RVT_N2 + evibO2_sum - q_loss) / (rho * cv);
                }
            }
            else // Flitti A, Pancheshnyi S 2009 model
            {
                ...
                *Ep_dot = P_ext;
                ...
                if (constPressure) {
                    ...
                    *dTdt = (-hdot_vol + P_ext - P_elec - q_loss) / (rho * cp);
                } else {
